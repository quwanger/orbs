...
public class TTSClient : MonoBehaviour {
	public const string SERVER_IP = "192.168.1.21";//"172.17.143.234";

    // Status
	bool isConnectionEstablished = false;
	public const int END_PACKET = 0;
	public const int INIT_CONNECTION = 1;
	public const int SERVER_REPLY_INIT = 2;
	public const int REPLY_CONNECTION = 3;
	public const int OBJ_UPDATE = 4;
	public const int SERVER_OBJECT_NOT_REGISTERED = 5;
	public const int SERVER_OBJECT_ALREADY_REGISTERED = 6;
	public const int REGISTER_OBJECT = 10;
	public const int SERVER_REGISTER_OK = 11;
	...
	void Update () {
        ...
        foreach (KeyValuePair<float, TTSNetworkObj> pair in networkObjects)
        {
            ...
            UpdateWriter.AddData(OBJ_UPDATE);
	...
	void OnApplicationQuit() {
		...
		UpdateWriter.ClearData();
        UpdateWriter.AddData(CLOSE_CONNECTION);
        SendPacket(UpdateWriter);
    ...
    private void InitConnection()
    {
        ...
        packet.AddData(INIT_CONNECTION);
        ...
        while (reading)
        {
            ...
            switch (command)
            {
                case END_PACKET:
                    reading = false;
                    break;
                case SERVER_REPLY_INIT:
                    // Server connection established
                    writer.AddData(REPLY_CONNECTION);
                ...
                case SERVER_OBJECT_ALREADY_REGISTERED:
                    objID = reader.ReadFloat();
    ...
    public void RegisterObject(TTSNetworkObj obj)
    {
		...
		UpdateWriter.AddData(REGISTER_OBJECT);
